...
public class SkillMatchService {
         ...
         * Best match: Teacher is 1-2 levels above learner
         */
        private double calculateSkillLevelGapScore(UserSkillLevel teacherLevel, UserSkillLevel learnerLevel) {
                ...
                if (gap == 1)
                        return 1.0; // Perfect: one level above
                if (gap == 2)
                        return 0.9; // Great: two levels above
                if (gap >= 3)
                        return 0.7; // Good: expert teaching beginner
                if (gap == 0)
                        return 0.5; // Okay: peer learning
                return 0.3; // Poor: learner more advanced than teacher
        }
